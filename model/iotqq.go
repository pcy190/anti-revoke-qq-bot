...
type SmallFaceMessage struct {
	Content string `json:"Content"`
	Hex     string `json:"Hex"`
	Index   int    `json:"Index"`
	Tips    string `json:"tips"`
}
type BigFaceMessage struct {
	Content string `json:"Content"`
	ForwordBuf     string `json:"ForwordBuf"`
	ForwordField   int    `json:"ForwordField"`
	Tips    string `json:"tips"`
...
type ReplayMessage struct {
	ReplayContent string `json:"ReplayContent"`
...
type Data struct {
	...
	EventData     EventData   `json:"EventData"` // used in OnEvents only
	EventMsg      EventMsg    `json:"EventMsg"`
	EventName     string      `json:"EventName"`
	FromUin       int         `json:"FromUin"` // used in friend msg
...
func GetCook() Cook {
	...
	if err != nil {
		fmt.Println("Unmarshal failed, info:", err)
	}
	return thecook
}

func SendPicAdvanced(ToUser int, SendToType int, Content string, FileMd5 string,PicUrl string,PicBase64Buf string) {
...
func SendPicByMd5(ToUser int, SendToType int, Content string, FileMd5 string) {
	tmp := make(map[string]interface{})
	tmp["toUser"] = ToUser
	tmp["sendToType"] = SendToType
	tmp["sendMsgType"] = "PicMsg"
...
func SendBigFace(ToUser int, SendToType int, ForwordBuf string) {
	tmp := make(map[string]interface{})
	tmp["toUser"] = ToUser
	tmp["sendToType"] = SendToType
	tmp["sendMsgType"] = "ForwordMsg"
	tmp["content"] = ""
	tmp["groupid"] = 0
	tmp["atUser"] = 0
	tmp["forwordBuf"]=ForwordBuf
	tmp["forwordField"]=6
	tmp["pwd"] = "mcoo"
	tmp1, _ := json.Marshal(tmp)
	resp, err := (http.Post("http://"+url1+"/v1/LuaApiCaller?funcname=SendMsg&timeout=10&qq="+qq, "application/json", bytes.NewBuffer(tmp1)))
...
func SendVoice(ToUser int, SendToType int, URL string,Content string) {
	tmp := make(map[string]interface{})
	tmp["toUser"] = ToUser
	tmp["sendToType"] = SendToType
	tmp["sendMsgType"] = "VoiceMsg"
	tmp["content"] = Content
	tmp["voiceUrl"] = "https://dds.dui.ai/runtime/v1/synthesize?voiceId=qianranfa&speed=0.7&volume=100&audioType=wav&text=" + url.PathEscape(URL)
	tmp["groupid"] = 0
	tmp["atUser"] = 0
	tmp["voiceBase64Buf"] = ""
	tmp["pwd"] = "mcoo"
